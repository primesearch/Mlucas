...
# Test for memrchr availability
echo -n "Checking for memrchr... "
cat > /tmp/test_memrchr_$$.c << 'EOF'
#define _GNU_SOURCE
#include <string.h>
int main(void) {
    const char *s = "test";
    void *p = memrchr(s, 't', 4);
    return p ? 0 : 1;
}
EOF

if "${CC:-gcc}" -o /tmp/test_memrchr_$$ /tmp/test_memrchr_$$.c >/dev/null 2>&1; then
	echo "yes"
	HAVE_MEMRCHR=1
	MEMRCHR_OBJ=''
else
	echo "no"
	HAVE_MEMRCHR=0
	MEMRCHR_OBJ='memrchr.o'
fi
rm -f /tmp/test_memrchr_$$ /tmp/test_memrchr_$$.c

# Generate config.h based on test results
cat > src/config.h << EOF
/*******************************************************************************
*                                                                              *
*   Configuration header for Mlucas (auto-generated by makemake.sh)           *
*                                                                              *
*******************************************************************************/
...
/*
 * memrchr() availability detected at build time.
 * HAVE_MEMRCHR is set to 1 if the system provides memrchr, 0 otherwise.
 */
#if $HAVE_MEMRCHR
