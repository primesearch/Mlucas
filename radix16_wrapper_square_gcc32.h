...
#define radix16_wrapper_square_gcc_h_included
	...
	"/* Real parts: */\n\t"\
		...
		"unpckhpd	0xc0(%%ebx)	,%%xmm6		/* a[j2+p12] gets read twice */\n\t"\
		"unpcklpd	0xc0(%%ebx)	,%%xmm4		/* a[jt+p12] */\n\t"\
		"movaps		%%xmm6		,0x160(%%ecx)	/* Store hi real in t23 */\n\t"\
	"/* Imag parts: */\n\t"\
		"movaps		0xd0(%%eax)	,%%xmm7\n\t"\
		"movaps		0xd0(%%eax)	,%%xmm5\n\t"\
		"unpckhpd	0xd0(%%ebx)	,%%xmm7\n\t"\
		"unpcklpd	0xd0(%%ebx)	,%%xmm5		/* a[jp+p12] */\n\t"\
	...
	"/* Real parts: */\n\t"\
		...
		"unpckhpd	0x80(%%ebx)	,%%xmm6		/* a[j2+p8 ] gets read twice */\n\t"\
		"unpcklpd	0x80(%%ebx)	,%%xmm4		/* a[jt+p8 ] */\n\t"\
		"movaps			%%xmm6	,0x120(%%ecx)	/* Store hi real in t19 */\n\t"\
	"/* Imag parts: */\n\t"\
		"movaps		0x90(%%eax)	,%%xmm7\n\t"\
		"movaps		0x90(%%eax)	,%%xmm5\n\t"\
		"unpckhpd	0x90(%%ebx)	,%%xmm7\n\t"\
		"unpcklpd	0x90(%%ebx)	,%%xmm5		/* a[jp+p8 ] */\n\t"\
...
"/* Do the p2,10 combo: */\n\t"\
	...
	"/* Real parts: */\n\t"\
		...
		"unpckhpd	0x20(%%ebx)	,%%xmm6		/* a[j2+p2 ] gets read twice */\n\t"\
		"unpcklpd	0x20(%%ebx)	,%%xmm0		/* a[jt+p2 ] */\n\t"\
		"movaps			%%xmm6	,0x100(%%ecx)	/* Store hi real in t9 +16 */\n\t"\
	"/* Imag parts: */\n\t"\
		"movaps		0x30(%%eax)	,%%xmm7\n\t"\
		"movaps		0x30(%%eax)	,%%xmm1\n\t"\
		"unpckhpd	0x30(%%ebx)	,%%xmm7\n\t"\
		"unpcklpd	0x30(%%ebx)	,%%xmm1		/* a[jp+p2 ] */\n\t"\
	...
	"/* Real parts: */\n\t"\
		...
		"unpckhpd	0xa0(%%ebx)	,%%xmm6		/* a[j2+p10] gets read twice */\n\t"\
		"unpcklpd	0xa0(%%ebx)	,%%xmm4		/* a[jt+p10] */\n\t"\
		"movaps			%%xmm6	,0x120(%%ecx)	/* Store hi real in t11+16 */\n\t"\
	"/* Imag parts: */\n\t"\
		"movaps		0xb0(%%eax)	,%%xmm7\n\t"\
		"movaps		0xb0(%%eax)	,%%xmm5\n\t"\
		"unpckhpd	0xb0(%%ebx)	,%%xmm7\n\t"\
		"unpcklpd	0xb0(%%ebx)	,%%xmm5		/* a[jp+p10] */\n\t"\
...
"/* Do the p6,14 combo - do p14 first so register assignments come out in same relative order as for p2,10 */\n\t"\
	...
	"/* Real parts: */\n\t"\
		...
		"unpckhpd	0xe0(%%ebx)	,%%xmm6		/* a[j2+p14] gets read twice */\n\t"\
		"unpcklpd	0xe0(%%ebx)	,%%xmm4		/* a[jt+p14] */\n\t"\
		"movaps			%%xmm6	,0x160(%%ecx)	/* Store hi real in t15+16 */\n\t"\
	"/* Imag parts: */\n\t"\
		"movaps		0xf0(%%eax)	,%%xmm7\n\t"\
		"movaps		0xf0(%%eax)	,%%xmm5\n\t"\
		"unpckhpd	0xf0(%%ebx)	,%%xmm7\n\t"\
		"unpcklpd	0xf0(%%ebx)	,%%xmm5		/* a[jp+p14] */\n\t"\
	...
	"/* Real parts: */\n\t"\
		...
		"unpckhpd	0x60(%%ebx)	,%%xmm6		/* a[j2+p6 ] gets read twice */\n\t"\
		"unpcklpd	0x60(%%ebx)	,%%xmm4		/* a[jt+p6 ] */\n\t"\
		"movaps			%%xmm6	,0x140(%%ecx)	/* Store hi real in t13+16 */\n\t"\
	"/* Imag parts: */\n\t"\
		"movaps		0x70(%%eax)	,%%xmm7\n\t"\
		"movaps		0x70(%%eax)	,%%xmm5\n\t"\
		"unpckhpd	0x70(%%ebx)	,%%xmm7\n\t"\
		"unpcklpd	0x70(%%ebx)	,%%xmm5		/* a[jp+p6 ] */\n\t"\
...
"/*	SSE2_RADIX4_DIF_4TWIDDLE(r17,r21,r19,r23, r17,c1): */\n\t"\
	"/* Do the p0,p8 combo: */\n\t"\
		...
		"																\n\t	mulpd   0x20(%%ebx)	,%%xmm4		/* a[jt+p8 ]*c8 */\n\t"\
		"subpd   	%%xmm3		,%%xmm0		/* xmm0 <- t1 */			\n\t	mulpd   0x20(%%ebx)	,%%xmm5		/* a[jp+p8 ]*c8 */\n\t"\
		"																\n\t	mulpd   0x30(%%ebx)	,%%xmm6		/* a[jt+p8 ]*s8 */\n\t"\
		"movaps		%%xmm0		,%%xmm2		/* xmm2 <- cpy t1 */		\n\t	mulpd	0x30(%%ebx)	,%%xmm7		/* a[jp+p8 ]*s8 */\n\t"\
		"																\n\t	addpd   %%xmm6	    ,%%xmm5		/* xmm5 <- it */\n\t"\
	...
	"/* Do the p4,12 combo: */\n\t"\
		...
		"\n\t"\
		"mulpd		    (%%ebx)	,%%xmm4		/* a[jt+p12]*c12 */\n\t"\
		"mulpd		    (%%ebx)	,%%xmm5		/* a[jp+p12]*c12 */\n\t"\
		"mulpd		0x10(%%ebx)	,%%xmm6		/* a[jt+p12]*s12 */\n\t"\
		"mulpd		0x10(%%ebx)	,%%xmm7		/* a[jp+p12]*s12 */\n\t"\
		"movl		%%eax,%%edx	/* r17 */\n\t"\
		"addpd		%%xmm6		,%%xmm5	/* xmm5 <- it */\n\t"\
		...
		"\n\t"\
		"mulpd		    (%%ebx)	,%%xmm4		/* a[jt+p4]*c4 */\n\t"\
		"mulpd		    (%%ebx)	,%%xmm5		/* a[jp+p4]*c4 */\n\t"\
		"mulpd		0x10(%%ebx)	,%%xmm6		/* a[jt+p4]*s4 */\n\t"\
		"mulpd		0x10(%%ebx)	,%%xmm7		/* a[jp+p4]*s4 */\n\t"\
...
"/*...Block 1: t1,9,17,25 */\n\t"\
		...
		"movaps		     (%%ebx)	,%%xmm2		/* t9  */\n\t"\
		"movaps		0x010(%%ebx)	,%%xmm3		/* t10 */\n\t"\
		"\n\t"\
		"subpd		     (%%ebx)	,%%xmm0		/* t9 =t1 -rt */\n\t"\
		"subpd		0x010(%%ebx)	,%%xmm1		/* t10=t2 -it */\n\t"\
		...
		"movaps		%%xmm0		,    (%%ebx)/* a[jt+p2 ], store in t9  */\n\t"\
		...
		"movaps		%%xmm4		,0x10(%%ebx)/* a[jp+p2 ], store in t10 */\n\t"\
		"\n\t"\
"/*...Block 3: t5,13,21,29 */\n\t"\
		"addl		$0x40,%%eax	/* r5  */	\n\t"\
		"addl		$0x40,%%ebx	/* r13 */	\n\t"\
		...
		"mulpd		(%%esi)		,%%xmm4	/* t21 = (t21-t22)*ISRT2 */\n\t"\
		"mulpd		(%%esi)		,%%xmm5	/* t22 = (t22+t21)*ISRT2 */\n\t"\
		"\n\t"\
		"addpd		0x190(%%eax)	,%%xmm6		/* t29+t30 */\n\t"\
		"subpd		0x180(%%eax)	,%%xmm7		/* t30-t29 */\n\t"\
		"mulpd		(%%esi)		,%%xmm6	/*  rt = (t29+t30)*ISRT2 */\n\t"\
		"mulpd		(%%esi)		,%%xmm7	/*  it = (t30-t29)*ISRT2 */\n\t"\
		"\n\t"\
...
"/*...Block 2: t3,11,19,27 */\n\t"\
		...
		"mulpd		(%%esi)		,%%xmm2	/* rt = (t11-t12)*ISRT2 */\n\t"\
		"mulpd		(%%esi)		,%%xmm3	/* it = (t12+t11)*ISRT2 */\n\t"\
		"\n\t"\
		"movaps		     (%%eax)	,%%xmm0		/* t3  */\n\t"\
...
"/*...Block 4: t7,15,23,31 */\n\t"\
		...
		"mulpd		(%%esi)		,%%xmm2	/* rt = (t15+t16)*ISRT2 */\n\t"\
		"mulpd		(%%esi)		,%%xmm3	/* it = (t16-t15)*ISRT2 */\n\t"\
		"\n\t"\
		"movaps		     (%%eax)	,%%xmm0		/* t7  */\n\t"\
	...
	__asm__ volatile (\
		...
		"addpd		%%xmm3,%%xmm4\n\t"\
		"\n\t"\
		"movaps		%%xmm5,0x90(%%ebx)\n\t"\
		"movaps		%%xmm4,0x80(%%ebx)\n\t"\
		"\n\t"\
		"movl		%[__c5],%%ecx\n\t"\
		"movl		%[__c13],%%edx\n\t"\
		"\n\t"\
		...
		"addpd		%%xmm5,%%xmm0\n\t"\
		"\n\t"\
		"movaps		%%xmm6,0xd0(%%ebx)\n\t"\
		"movaps		%%xmm0,0xc0(%%ebx)\n\t"\
		"\n\t"\
		...
		"addpd		%%xmm1,%%xmm6\n\t"\
		"\n\t"\
		"movaps		%%xmm7,0x30(%%ebx)\n\t"\
		"movaps		%%xmm6,0x20(%%ebx)\n\t"\
		"\n\t"\
		"movaps		     (%%eax),%%xmm6\n\t"\
		...
		"addpd		%%xmm1,%%xmm6\n\t"\
		"\n\t"\
		"movaps		%%xmm7,0xb0(%%ebx)\n\t"\
		"movaps		%%xmm6,0xa0(%%ebx)\n\t"\
		"\n\t"\
		"movl		%[__c7],%%ecx\n\t"\
		"movl		%[__c15],%%edx\n\t"\
		"\n\t"\
		...
		"addpd		%%xmm1,%%xmm2\n\t"\
		"\n\t"\
		"movaps		%%xmm4,0xf0(%%ebx)\n\t"\
		"movaps		%%xmm2,0xe0(%%ebx)\n\t"\
		"\n\t"\
		...
		"\n\t"\
		"movaps		%%xmm5,0xb0(%%ebx)\n\t"\
		"movaps		%%xmm4,0xa0(%%ebx)\n\t"\
		"\n\t"\
		"movl		%[__c6],%%ecx\n\t"\
		"movl		%[__c14],%%edx\n\t"\
		"\n\t"\
		...
		"\n\t"\
		"movaps		%%xmm2,0x70(%%ebx)\n\t"\
		"movaps		%%xmm7,0x60(%%ebx)\n\t"\
		"\n\t"\
		"movaps		%%xmm0,%%xmm4\n\t"\
		"movaps		%%xmm6,%%xmm5\n\t"\
		"mulpd		    (%%edx),%%xmm0\n\t"\
		...
		"\n\t"\
		"movaps		%%xmm6,0xf0(%%ebx)\n\t"\
		"movaps		%%xmm0,0xe0(%%ebx)\n\t"\
		:					/* outputs: none */\
		: [__add0] "m" (Xadd0)	/* All inputs from memory addresses here */\
		 ,[__add1] "m" (Xadd1)\
		 ,[__r1] "m" (Xr1)\
