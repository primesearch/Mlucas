...
// SIMD+SSE2 code only available for GCC build:
  ...
  // For Fermat-mod we use RADIX*4 = 4096 [note there is no LOACC option for this power-of-2 DFT] more
  // slots in AVX mode for the compact negacyclic-roots chained-multiply scheme. Add larger of the 2 numbers -
  // 4096 for AVX, 20 for SSE2 - to (half_arr_offset1024 + RADIX) to get SIMD value of radix1024_creals_in_local_store:
  #ifdef USE_AVX
	...
	const int radix1024_creals_in_local_store = 0x201c;	// (half_arr_offset1024 + RADIX) + 0x14 and round up to nearest multiple of 4
  #endif
...
/*...initialize things upon first entry: */
	...
	// Init-mode calls to these functions which maintain an internal local-alloc static store:
		...
		for(l = 0; l < 64; l++) {
	  #endif
			j = reverse(l,64)<<1;	// twid00-offsets are processed in BR order
