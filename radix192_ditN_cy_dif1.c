...
#define RADIX 192	// Use #define rather than const int to ensure it's really a compile-time const in the C sense
  ...
  // Add relevant number (half_arr_offset192 + RADIX) to get required value of radix192_creals_in_local_store:
  #ifdef USE_AVX
	const int half_arr_offset192 = 0x302 + (RADIX>>2);	// RADIX/4 vec_dbl slots for carries in AVX mode
	// (half_arr_offset192 + RADIX) + 68 and round up to nearest multiple of 4:
/*	const int radix192_creals_in_local_store = (half_arr_offset192 + RADIX + 68 + 3)&(~0x3);	<*** GCC [not Clang] gives 'error: initializer element is not constant'... */
	const int radix192_creals_in_local_store = (0x302 + (RADIX>>2) + RADIX + 68 + 3)&(~0x3); // <*** ...here is that workaround for that stupidity.
  #else
	const int half_arr_offset192 = 0x302 + (RADIX>>1);	// RADIX/2 vec_dbl slots for carries in SSE2 mode
	// (half_arr_offset192 + RADIX) = 20 and round up to nearest multiple of 4:
/*	const int radix192_creals_in_local_store = (half_arr_offset192 + RADIX + 20 + 3)&(~0x3); see above note in AVX decl. */
	const int radix192_creals_in_local_store = (0x302 + (RADIX>>1) + RADIX + 20 + 3)&(~0x3); // <*** ...here is that workaround for that stupidity.
  #endif
...
//		ASSERT(HERE, half_arr_offset == (uint32)(half_arr-sc_ptr), "half_arr_offset mismatches actual!");
	...
	// Init-mode calls to these functions which maintain an internal local-alloc static store:
		...
		nbytes = (int)cy - (int)cc0;	// #bytes in above sincos block of data
		tmp = cc0;
...
/*...The radix-192 pass is here.	*/
	...
	*/
		...
		jt = j1    ;	RADIX_64_DIF((double *)(t+0x00),i_offsets,1, (a+jt),(o_offsets+0x00),RE_IM_STRIDE);	// Inouts in t[00-63]
		jt = j1+p80;	RADIX_64_DIF((double *)(t+0x40),i_offsets,1, (a+jt),(o_offsets+0x40),RE_IM_STRIDE);	// Inouts in t[64-127]
		jt = j1+p40;	RADIX_64_DIF((double *)(t+0x80),i_offsets,1, (a+jt),(o_offsets+0x80),RE_IM_STRIDE);	// Inouts in t[128-191]
	}
}
