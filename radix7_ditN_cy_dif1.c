...
*/
	...
	static uint32 bw,sw,bjmodnini,p1,p2,p3,p4,p5,p6      ;
...
/*...change n7 and n_div_wt to non-static to work around a gcc compiler bug. */
	...
	{
		sprintf(cbuf,"FATAL: iter = %10d; NWT_BITS does not divide N/7 in radix7_ditN_cy_dif1.\n",iter);
...
/*...and combine those to complete the radix-7 transform and do the carries. Since the outputs would
    normally be getting dispatched to 7 separate blocks of the A-array, we need 7 separate carries.	*/
		...
		if(MODULUS_TYPE == MODULUS_TYPE_MERSENNE)
		{
			...
			cmplx_carry_norm_errcheck0(a1p0r,a1p0i,cy_r0,bjmodn0);
			cmplx_carry_norm_errcheck (a1p1r,a1p1i,cy_r1,bjmodn1,0x1);
			cmplx_carry_norm_errcheck (a1p2r,a1p2i,cy_r2,bjmodn2,0x2);
			cmplx_carry_norm_errcheck (a1p3r,a1p3i,cy_r3,bjmodn3,0x3);
			cmplx_carry_norm_errcheck (a1p4r,a1p4i,cy_r4,bjmodn4,0x4);
			cmplx_carry_norm_errcheck (a1p5r,a1p5i,cy_r5,bjmodn5,0x5);
			cmplx_carry_norm_errcheck (a1p6r,a1p6i,cy_r6,bjmodn6,0x6);
...
*/
	...
	*/
	...
	{
	    sprintf(cbuf,"FATAL: iter = %10d; nonzero exit carry in radix7_ditN_cy_dif1 - input wordsize may be too small.\n",iter);
	    if(INTERACT)fprintf(stderr,"%s",cbuf);
	    fp = fopen(   OFILE,"a");
	    fq = fopen(STATFILE,"a");
		fprintf(fp,"%s",cbuf);
...
/*...and combine those to complete the radix-7 transform and do the carries. Since the outputs would
    normally be getting dispatched to 7 separate blocks of the A-array, we need 7 separate carries.	*/
		...
		if(MODULUS_TYPE == MODULUS_TYPE_MERSENNE)
		{
			...
			/*...set0 is slightly different from others:	*/
			cmplx_carry_norm_nocheck0(a1p0r,a1p0i,cy_r0,bjmodn0);
			cmplx_carry_norm_nocheck (a1p1r,a1p1i,cy_r1,bjmodn1,0x1);
			cmplx_carry_norm_nocheck (a1p2r,a1p2i,cy_r2,bjmodn2,0x2);
			cmplx_carry_norm_nocheck (a1p3r,a1p3i,cy_r3,bjmodn3,0x3);
			cmplx_carry_norm_nocheck (a1p4r,a1p4i,cy_r4,bjmodn4,0x4);
			cmplx_carry_norm_nocheck (a1p5r,a1p5i,cy_r5,bjmodn5,0x5);
			cmplx_carry_norm_nocheck (a1p6r,a1p6i,cy_r6,bjmodn6,0x6);
