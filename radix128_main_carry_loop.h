...
/* Gather the needed data (128 64-bit complex, i.e. 256 64-bit reals) and do 8 twiddleless length-16 subtransforms: */
	...
	tmp = cc16-1;	// Start of isrt2/cc16/ss16 triplet needed for radix-16 SSE2 DFT macros [use instead of isrt2 here]
	tm1 = r00;
	for(l = 0; l < 8; l++) {
		...
		SSE2_RADIX16_DIT_0TWIDDLE(add0,add1,add2,add3,add4,add5,add6,add7,add8,add9,adda,addb,addc,addd,adde,addf, tmp,
		tm1,OFF1,OFF2,OFF3,OFF4); tm1 += 32;
	}
...
*/
	...
	for(l = 1; l < 16; l++) {
		...
		jt = j1 + k2; jp = j2 + k2;
		addr = DFT128_TWIDDLES[l-1]; addi = addr+1;	// Pointer to required row of 2-D twiddles array
...
// NOTE that unlike the RADIX_08_DIF_OOP() macro used for pass 1 of the radix-64 DFT, RADIX_16_DIF outputs are IN-ORDER rather than BR:
	...
	for(l = 0; l < 8; l++) {
		...
		SSE2_RADIX16_DIF_0TWIDDLE  (tm2,OFF1,OFF2,OFF3,OFF4, tmp, tm1,add1,add2,add3,add4,add5,add6,add7,add8,add9,adda,addb,addc,addd,adde,addf);
	#else
		SSE2_RADIX16_DIF_0TWIDDLE_B(tm2,OFF1,OFF2,OFF3,OFF4, tmp, tm1);
	#endif
		tm1 += 32;
	}
