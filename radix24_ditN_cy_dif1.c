...
//	printf("radix32_ditN_cy_dif1 end  ; #tasks = %d, #free_tasks = %d\n", tpool->tasks_queue.num_tasks, tpool->free_tasks_queue.num_tasks);
	...
	*/
	...
	for(ithread = 0; ithread < CY_THREADS; ithread++)
	{
		...
		{								j1=j+p08;					j2=j+p16;
			a[j     ] *= radix_inv;		a[j1    ] *= radix_inv;		a[j2    ] *= radix_inv;
			a[j +p01] *= radix_inv;		a[j1+p01] *= radix_inv;		a[j2+p01] *= radix_inv;
			a[j +p02] *= radix_inv;		a[j1+p02] *= radix_inv;		a[j2+p02] *= radix_inv;
			a[j +p03] *= radix_inv;		a[j1+p03] *= radix_inv;		a[j2+p03] *= radix_inv;
			a[j +p04] *= radix_inv;		a[j1+p04] *= radix_inv;		a[j2+p04] *= radix_inv;
			a[j +p05] *= radix_inv;		a[j1+p05] *= radix_inv;		a[j2+p05] *= radix_inv;
			a[j +p06] *= radix_inv;		a[j1+p06] *= radix_inv;		a[j2+p06] *= radix_inv;
			a[j +p07] *= radix_inv;		a[j1+p07] *= radix_inv;		a[j2+p07] *= radix_inv;
		}
	}
}	/* endfor(outer) */
