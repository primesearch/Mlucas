	...
	*/
	...
	"popl %%ebx	\n\t"\
		...
		: [__fqinv0] "m" (fqinv0)	/* All inputs from memory addresses here */\
		 ...
		 ,[__pshift] "m" (pshift)	\
		 ,[__j]		 "m" (j)		\
		: "cc","memory","eax",/*"ebx",*/"ecx","edx","xmm0","xmm1","xmm2","xmm3","xmm7"	/* Clobbered registers */\
	);
	}	/* for(j...) */
...
)
{
	...
	*/
	...
	__asm__ volatile (\
		"movl	%[__fq0],%%eax						\n\t"\
		"movl	%[__two26f],%%esi					\n\t"/* two26i in next-higher slot */\
		"movaps	    (%%eax),%%xmm0	/* fq0 */		\n\t	movaps	0x10(%%eax),%%xmm4	/* hq0 */		\n\t"\
		"movaps	0x20(%%eax),%%xmm1	/* fq1 */		\n\t	movaps	0x30(%%eax),%%xmm5	/* hq1 */		\n\t"\
		"movaps	0x40(%%eax),%%xmm2	/* fq2 */		\n\t	movaps	0x50(%%eax),%%xmm6	/* hq2 */		\n\t"\
		...
		"mulpd	0x10(%%esi),%%xmm1					\n\t	mulpd	0x10(%%esi),%%xmm5						\n\t"\
		"mulpd	0x10(%%esi),%%xmm2					\n\t	mulpd	0x10(%%esi),%%xmm6						\n\t"\
		"movaps	%%xmm0,    (%%eax)	/* fq0/2^26 */	\n\t	movaps	%%xmm4,0x10(%%eax)	/* hq0/2^26 */	\n\t"\
		"movaps	%%xmm1,0x20(%%eax)	/* fq1/2^26 */	\n\t	movaps	%%xmm5,0x30(%%eax)	/* hq1/2^26 */	\n\t"\
		"movaps	%%xmm2,0x40(%%eax)	/* fq2/2^26 */	\n\t	movaps	%%xmm6,0x50(%%eax)	/* hq2/2^26 */	\n\t"\
		"movaps	%%xmm3,0x60(%%eax)	/* fqhi52 */	\n\t	movaps	%%xmm7,0x70(%%eax)	/* hqhi52 */	\n\t"\
		:					/* outputs: none */\
		: [__fq0] "m" (fq0)	/* All inputs from memory addresses here */\
		 ,[__two26f] "m" (two26f)\
		: "cc","memory","eax","esi","xmm0","xmm1","xmm2","xmm3","xmm4","xmm5","xmm6","xmm7"		/* Clobbered registers */\
	);
	...
	CVT_UINT78_3WORD_DOUBLE(qinv3 ,*iqinv0,*iqinv1,*iqinv2);
		...
		__asm__ volatile (\
			...
			: [__fqinv0] "m" (fqinv0)	/* All inputs from memory addresses here */\
			 ,[__two26i] "m" (two26i)	\
			: "cc","memory","eax","esi","xmm0","xmm1","xmm2","xmm3","xmm4","xmm5","xmm6"	/* Clobbered registers */\
		);
...
//exit(0);
		...
		__asm__ volatile (\
		"pushl %%ebx	\n\t"/* Explicit save/restore of PIC register */\
		/* SQR_LOHI78_3WORD_DOUBLE_q4(fx, flo,fhi). Inputs flo0,1,2 enter in xmm0,1,2 [lcol] and xmm4,5,6 [rcol]: */\
			"movl	%[__fx0],%%eax	\n\t"\
			"movaps	    (%%eax),%%xmm0		\n\t	movaps	0x10(%%eax),%%xmm4	\n\t"\
			"movaps	0x20(%%eax),%%xmm1		\n\t	movaps	0x30(%%eax),%%xmm5	\n\t"\
		...
		/* fx0,1,2 assumed in xmm0,1,2 on loop entry */\
			...
			"movaps	%%xmm3,0x80(%%eax)		\n\t	movaps	%%xmm7,0x90(%%eax)	\n\t"/* Recall: fx0,1 in eax+0x[0,2]0, fx2 in xmm1. */\
		/* MULL78 section below needs flo0,1,2 in xmm0,1,2: */\
		/* flo = MULL78_3WORD_DOUBLE_q4(flo, fqinv): */\
			"movl	%[__fqinv0],%%edx	\n\t"\
			/* Digit 0: */\
			"movaps	    (%%eax),%%xmm0		\n\t	movaps	0x10(%%eax),%%xmm4	\n\t"/* Reload flo0 */\
			"movaps	%%xmm1,%%xmm2			\n\t	movaps	%%xmm5,%%xmm6		\n\t"/* Move flo2 into xmm2 */\
			...
			"subpd	%%xmm3,%%xmm2			\n\t	subpd	%%xmm7,%%xmm6		\n\t"/* fprod2 -= fcy */\
			"mulpd	-0x10(%%ebx),%%xmm2		\n\t	mulpd	-0x10(%%ebx),%%xmm6	\n\t"/* fprod2 *= two26f */\
		/* flo = MULH78_3WORD_DOUBLE_q4(flo, fq): flo0,1,2 enter in xmm4,1,2. *** IN THE COMMENTS X = Q, Y = LO: ***/\
			"movl	%[__fq0],%%edx		\n\t"\
			/* Digit 0: */\
			"movaps	    (%%eax),%%xmm0		\n\t	movaps	0x10(%%eax),%%xmm4	\n\t"/* Reload y0 = fprod0 */\
			"movaps	0x40(%%eax),%%xmm1		\n\t	movaps	0x50(%%eax),%%xmm5	\n\t"/* Reload y1 = fprod1 [in flo2 slot] */\
			"movaps	%%xmm0,%%xmm3			\n\t	movaps	%%xmm4,%%xmm7		\n\t"/* xmm3 = cpy of y0 */\
			"mulpd	    (%%edx),%%xmm0		\n\t	mulpd	0x10(%%edx),%%xmm4	\n\t"/* fprod0 = y0*x0 */\
			"mulpd	    (%%ebx),%%xmm0		\n\t	mulpd	    (%%ebx),%%xmm4	\n\t"/* CY    *= two26i */\
			/* Digit 1: */\
		...
		/* Precompute partial products needed for upper half: */\
			...
			"mulpd	-0x10(%%ebx),%%xmm1		\n\t	mulpd	-0x10(%%ebx),%%xmm5	\n\t"/* fprod4 *= two26f */\
		/* If h < l, calculate h-l+q; otherwise h-l. Use leading 52 bits to approximate the full 78-bit compare. Result is in [0, q). */\
			"movaps	0x80(%%eax),%%xmm2		\n\t	movaps	0x90(%%eax),%%xmm6	\n\t"/* fhi, top 52 bits */\
			"mulpd	-0x10(%%ebx),%%xmm3		\n\t	mulpd	-0x10(%%ebx),%%xmm7	\n\t"/* flo2 *= two26f */\
			"addpd	%%xmm1,%%xmm3			\n\t	addpd	%%xmm5,%%xmm7		\n\t"/* flo, top 52 bits */\
			...
			"movaps	-0x10(%%ebx),%%xmm3		\n\t	movaps	-0x10(%%ebx),%%xmm7	\n\t"/* 2^26 */\
			"mulpd	    (%%edx),%%xmm3		\n\t	mulpd	0x10(%%edx),%%xmm7	\n\t"/* fq, low 26 bits */\
			"movaps	0x60(%%edx),%%xmm0		\n\t	movaps	0x70(%%edx),%%xmm4	\n\t"/* qhi52 :+ fq, top 52 bits */\
			"andpd	(%%eax),%%xmm0			\n\t	andpd	0x10(%%eax),%%xmm4	\n\t"/* qhi52 & bitmask */\
			"addpd	%%xmm0,%%xmm2			\n\t	addpd	%%xmm4,%%xmm6		\n\t"/* xhi = (h-l)hi + (qhi52 & bitmask) */\
			"movaps	%%xmm3,0x20(%%eax)		\n\t	movaps	%%xmm7,0x30(%%eax)	\n\t"/* Store qlo26 to free up a register */\
			"andpd	(%%eax),%%xmm3			\n\t	andpd	0x10(%%eax),%%xmm7	\n\t"/* qlo26 &= bitmask */\
			"addpd	%%xmm3,%%xmm1			\n\t	addpd	%%xmm7,%%xmm5		\n\t"/* xlo = (h-l)lo + (qlo26 & bitmask) */\
			/* xhi52,xlo26 in xmm2,xmm1; qhi52,qlo26 & bitmask in xmm0,3: */\
		/* if((pshift >> j) & (uint64)1) { */\
			"movl	%[__pshift],%%eax	\n\t"\
			"movl	%[__j],%%ecx		\n\t"\
			"shrl	%%cl,%%eax		\n\t"\
			"andl	$0x1,%%eax		\n\t"\
		"je twopmodq78_3wdq4_gcc32	\n\t"\
			"movl	%[__fx0],%%eax		\n\t"\
			"addpd	%%xmm2,%%xmm2			\n\t	addpd	%%xmm6,%%xmm6		\n\t"/* 2.[top 52 bits] */\
			"addpd	%%xmm1,%%xmm1			\n\t	addpd	%%xmm5,%%xmm5		\n\t"/* 2.[low 26 bits] */\
			/* If x > q, subtract q: */\
...
//printf("SSE2: j = %2d, x = %20.10f,%20.10f,%20.10f\n",j,*fx0,*fx1,*fx2);
		}	/* for(j...) */
//exit(0);
