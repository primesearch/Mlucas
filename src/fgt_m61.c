...
uint64 prodq8(const uint64 x, const uint64 y)
{
	...
	MUL_LOHI64(x,y,  lo, hi);
#endif
	ASSERT(HERE, (lo & 7) == 0, "FATAL: product not divisible by 8 in PRODQ8!");
//if(hi + (lo >> 3) > q2) fprintf(stderr, "PRODQ8 inputs: %llu,%llu, outputs: %llu,%llu, result = %llu\n",x,y,lo,hi,hi + (lo >> 3));
//	ASSERT(HERE, hi + (lo >> 3) <= q2, "FATAL: result out of range in PRODQ8!");
...
{
	ASSERT(HERE, (x >> 61) == 0, "FATAL: x out of range in MUL_BY_3BIT!");
	return a * x;
}
...
uint64 rmul_modq(const uint64 x, const uint64 y)
{
	...
	ASSERT(HERE, bd_lo <= (bd_hi << 3) + bd_lo, "FATAL: overflow of b*d(lo + hi>>3) summand!");
	bd_modq = qreduce((bd_hi << 3) + bd_lo);

	ay = mul_by_3bit((x >> 58),y);
	cb = mul_by_3bit((y >> 58), (x & two58m1));
	ASSERT(HERE, cb <= ay+cb, "FATAL: overflow of ay+cb summand!");
	bd_modq = qreduce((bd_hi << 3) + bd_lo);
...
{
	uint64 t00,t01,t11;
	ASSERT(HERE, (a0 <= bb && a1 <= bb && b0 <= q && b1 <= q), "FATAL: CMUL_MODQ input out of range!");
	...
	*xout = t00 - t11 + q2;		// xout in [0, 4q]
	ASSERT(HERE, *xout <= q4, "FATAL: xout > 4q in CMUL_MODQ!");
	...
	*yout = prodq8(a1, b0<<3) + t01;	// a1    in [0, B]: prodq8hi(a1,8b0) in [0, q], t10 in [0, 2q]; yout in [0, 4q]
	ASSERT(HERE, *yout <= q4, "FATAL: yout > 4q in CMUL_MODQ!");
#else
	// Karatsuba variant:
	...
	*yout = qreduce(t01 - t00 - t11 + q4);	// t01 in [0, 5q] but t01-t00-t11 in [0,4q], so no overflow in t01-t00-t11+q4.
	ASSERT(HERE, t01 <= (q4 + q), "FATAL: t01 > 5q in Karatsuba-part of CMUL_MODQ!");
...
{
	uint64 t00,t01,t11;
	ASSERT(HERE, ( a0 <= bb && a1 <= bb ), "FATAL: CMUL_MODQ8 A-input out of range!");
	ASSERT(HERE, (!(b0 & 7) && !(b1 & 7)), "FATAL: CMUL_MODQ8 B-input not divisible by 8!");
...
void csqr_modq(const uint64 a0, const uint64 a1, uint64*xout, uint64*yout)
{
	...
	*xout = prodq8((a0 + a1)<<1, (a0 - a1 + q)<<2);	// prodq8hi( 2(a0+a1) , 4(a0-a1+q) ) in [0,4q]; xout in [0,5q]
	ASSERT(HERE, *xout <= (q4+q), "FATAL: xout >= 5q in CSQR_MODQ!");

	*yout = prodq8(a0<<2, a1<<2);					// prodq8hi(     4*a0 , 4*a1       ) in [0,2q]; yout in [0,3q]
	ASSERT(HERE, *yout < (q4-q), "FATAL: yout > 3q in CSQR_MODQ!");
}
