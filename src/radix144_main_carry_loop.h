...
for(k=1; k <= khi; k++)	/* Do n/(radix(1)*nwt) outer loop executions...	*/
{
	...
	normally be getting dispatched to RADIX separate blocks of the A-array, we need 28 separate carries.	*/
	  ...
	  #ifdef CARRY_16_WAY
		const uint32 nloop = RADIX>>4;		incr = 3;
	  #else
		const uint32 nloop = RADIX>>3;		incr = 3;
	  #endif
		...
		for(l = 0; l < RADIX>>2; l++) {
			...
			AVX_cmplx_carry_norm_errcheck_X4(tm1,add1,add2,add3,tmp,itmp,half_arr,i,n_minus_silp1,n_minus_sil,sign_mask,sinwt,sinwtm1,sse_bw,sse_n,sse_sw, add0,p1,p2,p3);
