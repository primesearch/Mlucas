...
void radix16_wrapper_ini(int n, int radix0, int iblock, int nradices_prim, int radix_prim[], int ws_i[], int ws_j1[], int ws_j2[], int ws_j2_start[], int ws_k[], int ws_m[], int ws_blocklen[], int ws_blocklen_sum[])
{
	...
	if(iblock == 0)		/* j1 is the real-array index (double the complex-array index) of the 1st element of each floating pair. */
	{
		...
		k            =  0;
		blocklen     = 16;	/* This is half the actual complex blocklength, since we process two complex data for each value of the loop index L. */
		blocklen_sum =  0;
...
========================
So when j1 = 0 on entry we exit immediately via:
		if(j1 && ((uint64)j1*radix0)%n == 0)
		{
