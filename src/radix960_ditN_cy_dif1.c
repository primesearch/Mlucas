...
/*...initialize things upon first entry: */
	...
	if(first_entry)
	{
		...
		*/
		...
		ASSERT(HERE, (radix960_creals_in_local_store << l2_sz_vd) >= ((long)half_arr - (long)r00) + (20 << l2_sz_vd), "radix960_creals_in_local_store checksum failed!");
...
/*...The radix-960 pass is here.	*/
	...
	*/
		...
		*	1e0,0e0,3a0,2a0,1a0,0a0,360,260,160,060,320,220,120,020,2e0 + f	<*** sub 0x10 from leading term whenever lo part passes through 0!
		...
		*	2d0,1d0,0d0,390,290,190,090,350,250,150,050,310,210,110,010 + f	<*** sub 0x10 from leading term whenever lo part passes through 0!
		...
		*	000,2c0,1c0,0c0,380,280,180,080,340,240,140,040,300,200,100 + f	<*** sub 0x10 from leading term whenever lo part passes through 0!
		...
		For the LHS we need 2 ingredients:
			[1] A formula yielding the leftmost elt of each row. Init k = 0x3c0, decrementing by 0x2c0 each row
				(mod 960 = 0x3c0) and further -= 0x10 whenever lo part passes through 0 gives the desired terms for i > 0;
		...
		*/
		...
		// [2] here:
			...
			k -= 0x2c + ((l&0xf) == 0); //<*** further decr leading term whenever lo part passes through 0
			k += (-(k < 0))&0x3c;
		#else
