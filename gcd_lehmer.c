	...
	else if(len_diff > 0)
	{
		...
		*/
		...
		if(j >= (max_bits>>1))
		{
			...
			//	printf("Input  = %s\n",&str_10k[__convert_mi64_base10_char(str_10k, 10<<10, uv_ptr[k2], lenV)]);
			}
		#endif
...
GCD_DONE:
	...
	{
		printf("GCD = %s\n", &string0[convert_mi64_base10_char(string0, uv_ptr[k1], lenU)]);
	}
...
// 6/08/2012: Trial-factor of mm127 residue check for Paul L:
	...
	printf("Res127 = %s\n", &string0[convert_mi64_base10_char(string0, (uint64*)&x192, lenX)]);
exit(0);
#endif
...
rem = mi64_div_by_scalar64(x, mod, vec_len, y);
ASSERT(HERE, rem == mi64_div_by_scalar64_u2(x, mod, vec_len, y), "mi64_div_by_scalar64 Test #0.a!");
ASSERT(HERE, rem == mi64_div_by_scalar64_u4(x, mod, vec_len, y), "mi64_div_by_scalar64 Test #0.b!");
...
ASSERT(HERE, mi64_div_by_scalar64(x, mod, lenX, y) == rem
	...
	*/
	...
	{
		mod = rng_isaac_rand() | 1;	// Current loop-foled DIV only allows odd modulus
		rem   = mi64_div_by_scalar64   (x, mod, i, y);
		tmp64 = mi64_div_by_scalar64_u4(x, mod, i, z);
		if(rem != tmp64) {
			printf("len = %u!\n",i);
		}
//		ASSERT(HERE, rem == tmp64, "GCD self-test #DIV");
		ASSERT(HERE, mi64_cmp_eq(y,z,i), "GCD self-test #DIV");
	}
	/********************************************************************************/
	...
	*/
	...
	for(j = 0; testp128[j].d0 != 0; ++j)
	{
		...
		/*
		fprintf(stderr, "mi64_pprimeF: testing p = %s\n", &char_buf[convert_uint128_base10_char(char_buf, testp128[j])]);
		*/
	...
	*/
	...
	for(j = 0; testp192[j].d0 != 0; ++j)
	{
		...
		/*
		fprintf(stderr, "mi64_pprimeF: testing p = %s\n", &char_buf[convert_uint192_base10_char(char_buf, testp192[j])]);
		*/
	...
	*/
	...
	ASSERT(HERE, STREQ(&char_buf[convert_mi64_base10_char(char_buf, x, lenX)], "627057063764139831929324851379409869378845668175598843037877190478889006888518431438644711527536922839520331484815861906173161536477065546885468336421475511783984145060592245840032548652210559519683510271"), "GCD self-test #8");
	/* q677: */
	u[2] =    157590042578912ull; u[1] = 10558642444782195772ull; u[0] =   329809049266961143ull;	lenU = 3;
	ASSERT(HERE, STREQ(&char_buf[convert_mi64_base10_char(char_buf, u, lenU)], "53625112691923843508117942311516428173021903300344567"), "GCD self-test #8");
...
//	mi64_setlen(u, lenU, lenX);	only needed for legacy mi64_div_binary function
	...
	ASSERT(HERE, STREQ(&char_buf[convert_mi64_base10_char(char_buf, w, lenX)], "11693347245097298120823316616845179432964234640294687969094035694006198722115249470397309583454281704888388908348322516169275641497746779501202976102713"), "GCD self-test #8");
	ASSERT(HERE, mi64_iszero(z, lenX) == 1, "GCD div-test #8.pr1");
	ASSERT(HERE, STREQ(&char_buf[convert_mi64_base10_char(char_buf, z, lenX)], "0"), "GCD self-test #8");
	...
	ASSERT(HERE, STREQ(&char_buf[convert_mi64_base10_char(char_buf, u, lenU)], "166388228042876887900053740282475640168826969539724862936313620741285381200003507633306121185343797304769"), "GCD self-test #8");
//	mi64_setlen(u, lenU, lenX);	only needed for legacy mi64_div_binary function
	mi64_div(x, u, lenX, lenU, w, z);	/* Check divisibility directly - quotient returned in w, remainder in z */
	ASSERT(HERE, mi64_iszero(z, lenX) == 1, "GCD div-test #8.pr2");
...
//	mi64_setlen(u, lenU, lenX);	only needed for legacy mi64_div_binary function
	mi64_div(x, u, lenX, lenU, w, z);	/* Check divisibility directly - quotient returned in w, remainder in z */
	ASSERT(HERE, mi64_iszero(z, lenX) == 1, "GCD div-test #8.pr3");
...
//	mi64_setlen(u, lenU, lenX);	only needed for legacy mi64_div_binary function
	mi64_div(x, u, lenX, lenU, w, z);	/* Check divisibility directly - quotient returned in w, remainder in z */
	ASSERT(HERE, mi64_iszero(z, lenX) == 1, "GCD div-test #8.pr4");
