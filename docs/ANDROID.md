...
5. Install build dependencies in the Ubuntu shell: `sudo apt -y install git wget python3 python3-requests build-essential libgmp-dev libhwloc-dev` (see [Prerequisites](../README.md#prerequisites) for what each is for).
6. Clone and build Mlucas as in the main [Build](../README.md#build) instructions — `bash makemake.sh` will auto-detect ASIMD support, or fall back to scalar-double mode on a non-SIMD device.
